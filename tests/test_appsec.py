...
class TestThreatModeling:
    ...
    def test_analyzer_initialization(self, mock_repo):
        """Test ThreatAnalyzer can be initialized."""
        from threat_modeling import ThreatAnalyzer

        analyzer = ThreatAnalyzer(str(mock_repo))
        assert analyzer.repo_path == Path(mock_repo)
        assert analyzer.threats == []
        assert analyzer.components == []

    def test_basic_analysis(self, mock_repo):
        """Test basic threat model generation."""
        from threat_modeling import ThreatAnalyzer
        ...
        # Verify structure
        assert 'architecture' in threat_model
        assert 'attack_surface' in threat_model
        assert 'stride_analysis' in threat_model
        assert 'threat_scenarios' in threat_model
        assert 'summary' in threat_model

        # Verify STRIDE categories exist
    ...
    def test_analysis_with_findings(self, mock_repo):
        """Test threat model with security findings."""
        from threat_modeling import ThreatAnalyzer
        ...
        # Should have threat mapped to STRIDE
        assert threat_model['summary']['total_threats'] > 0
        assert len(threat_model['stride_analysis']['Tampering']) > 0

    def test_export_functionality(self, mock_repo, temp_dir):
        """Test threat model export to files."""
        from threat_modeling import ThreatAnalyzer
        ...
        exported = analyzer.export_threat_model(threat_model, str(temp_dir))

        # Verify files were created
        assert 'json' in exported
        assert 'markdown' in exported
        assert 'diagram' in exported
        assert Path(exported['json']).exists()
        assert Path(exported['markdown']).exists()
        assert Path(exported['diagram']).exists()

    def test_mermaid_diagram_generation(self, mock_repo):
        ...
        diagram = analyzer.generate_mermaid_diagram(threat_model)

        # Verify Mermaid syntax
        assert diagram.startswith('```mermaid')
        assert diagram.endswith('```')
        assert 'graph TB' in diagram
        assert 'Trust Boundary' in diagram

    def test_empty_repo_handling(self, temp_dir):
        ...
        analyzer = ThreatAnalyzer(str(empty_repo))
        threat_model = analyzer.analyze([])
    ...
    def test_attack_surface_calculation(self, mock_repo):
        ...
        assert summary['attack_surface_score'] in ['LOW', 'MEDIUM', 'HIGH']
